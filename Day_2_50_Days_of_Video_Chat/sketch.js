...
function setup() {
  ...
  myVideo = createCapture(constraints, 
    function(stream) {
	  let p5l = new p5LiveMedia(this, "CAPTURE", stream, "Day 2 - 50 Days of Video Chat")
  ...
  if (loudest != -1) {
      for (const vl in otherVideos) {
        if (loudest == vl) {
          ...
          otherVideos[loudest].loudestCount++;
          otherVideos[loudest].text.innerHTML = otherVideos[loudest].loudestCount;
        } else {
